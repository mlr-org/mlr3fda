...
test_that("PipeOpFlatFun input validation works", {
  ...
  # grid is not union, minmax, intersect
  ...
  # grid is empty
  expect_error(po("flatfun", grid = c()))
})
...
test_that("PipeOpFlatFun works with minmax", {
  # tfr works
  dt = data.table(
    id = rep(1:2, each = 5),
    arg = rep(1:5, 2),
...
test_that("PipeOpFlatFun works with intersect", {
  ...
  pop = po("flatfun", grid = "intersect")
  task_flat = pop$train(list(task))[[1L]]
  expected = data.table(y = 1:2, f_1 = c(1, 3), f_2 = c(5, 10), f_3 = c(7, 12))
  expect_set_equal(c("f_1", "f_2", "f_3"), task_flat$feature_names)
  expect_equal(task_flat$data(), expected)
...
test_that("PipeOpFlatFun works with custom grid", {
  # tfr works
  dt = data.table(
    id = rep(1:2, each = 5),
    arg = rep(1:5, 2),
  ...
  dt = data.table(y = 1:2, f = f)
  task = as_task_regr(dt, target = "y")
  pop = po("flatfun", grid = 3:5)
  task_flat = pop$train(list(task))[[1L]]
  expected = data.table(y = 1:2, f_1 = c(5, 10), f_2 = c(5, 2), f_3 = c(7, 12))
  expect_set_equal(c("f_1", "f_2", "f_3"), task_flat$feature_names)
  expect_equal(task_flat$data(), expected)

  # outside of range
  pop = po("flatfun", grid = 3:7)
  ...
  pop = po("flatfun", grid = 3:5)
  task_flat = pop$train(list(task))[[1L]]
  expected = data.table(y = 1:2, f_1 = c(5, 10), f_2 = c(5, 11), f_3 = c(7, 12))
  expect_set_equal(c("f_1", "f_2", "f_3"), task_flat$feature_names)
  expect_equal(task_flat$data(), expected)
