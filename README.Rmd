...
This is achieved by adding support for functional feature types and providing preprocessing `PipeOp`s that operate on functional columns.
...
content = content[map_lgl(tags, \(t) "fda" %in% t), .(key, label, packages, tags)]
content[, packages := map(packages, \(x) setdiff(x, c("mlr3pipelines", "mlr3fda")))]
content[, `:=`(
  key = sprintf("[%1$s](https://mlr3fda.mlr-org.com/dev/reference/mlr_pipeops_%1$s)", key),
  packages = map_chr(packages, function(pkg) {
    toString(fifelse(
      pkg %in% c("stats", "graphics", "datasets"),
      pkg,
...
The development of this R-package was supported by Roche Diagnostics R&D.
